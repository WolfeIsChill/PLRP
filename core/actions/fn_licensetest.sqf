...
["You are starting a Drivers Test.. Do not go over 70kmh - also, put on your seatbelt!",false] spawn domsg;
playSound "racebeep1";
sleep 1;
["You are starting a Drivers Test.. Do not go over 70kmh - also, put on your seatbelt!",false] spawn domsg;
playSound "racebeep1";
...
while{_totaltest > 0} do {
	...
	["Look around the city for your next checkpoint!",false] spawn domsg;
	...
	while{true} do {
		...
		_timer = _timer + 1;
		uisleep 1;
		hint parsetext format["<img size='1' image='\OL_Textures\images\info.paa'/> <t color='#FFCC00'><t size='0.75'>JOB:</t><br/> You have taken %1 seconds.",_timer];
	};
	playSound "racebeep1";
