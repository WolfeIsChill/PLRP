...
["Lets do some yard work! (Don't Move for 30 Seconds!)", false] spawn domsg;

if (!life_is_arrested) exitwith {
	["You are not arrested", false] spawn domsg;
};
...
while {true} do {
	...
	if(_cme > 30) exitwith {
		...
		[format ["You earned $50 - NEW BAIL SET: %1", life_bail_amount], false] spawn domsg;
	};
};
