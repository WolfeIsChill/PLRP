...
_adminKey 			= if (count (actionKeys "User1") == 0) then { 7 } else { (actionKeys "User1") select 0 };
_pgspawnKey 		= if (count (actionKeys "User2") == 0) then { 8 } else { (actionKeys "User2") select 0 };
_checkpointKey  = if (count (actionKeys "User3") == 0) then { 10 } else { (actionKeys "User3") select 0 };
...
switch (_code) do
{
		...
		if(_shift) then
		{
			if( vehicle player != player ) then
			{
				...
				if(_locked == 2) then {
					_handled = true;
					[] spawn fnc_doorkick;
					["You just kicked out the doors of this vehicle..", false] spawn domsg;
				};
			};
		};
	};
	...
	case _adminKey: {
			if (call PLRP_Staff_Rank > 1) then { [] call admin_fnc_openMenu };
			_handled = true;
	};

	case _pgspawnKey: {
			if (call PLRP_Staff_Rank == 5) then { createDialog "balca_debug_main" };
			_handled = true;
	};

	case _checkpointKey: {
		if (playerSide == west) then {
			[] call cp_fnc_openMenu;
		};
		if (player getVariable "CarryingCP") then {
			[] call cp_fnc_place;
		};
	};
		...
		if(_shift && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && !(cursorTarget getVariable["dead",FALSE]) && cursorTarget distance player < 3 && vehicle player == player) then
		{
			if((animationState cursorTarget) != "Incapacitated" && !life_knockout && !(player getVariable["restrained",false]) && !(player getVariable["tied",false]) && !life_istazed && !life_paintball && (stance player) != "PRONE") then
			{
				if(side player == west) then {
					...
					if(time - life_knockOutCD < 2) exitWith {["You don't have a good enough grip to knock someone out yet",false] spawn domsg};
	...
	case 21:
	{
		...
		if(count _radios > 0 && ([(call TFAR_fnc_activeSwRadio),"OL_Tablet"] call TFAR_fnc_isSameRadio) ) then {
			if(life_battery < 1) then {
				if ( !_alt && !_ctrlKey ) then {
					["No Battery. CTRL+Y to open food menu | ALT+Y to open key menu | CTRL+8 to sync player data", false] spawn doquickmsg;
					_stop = true;
					_handled = true;
					[] spawn life_fnc_AntiSpam2;
				};
		...
		if(count _radios == 0) then {
			if( !_alt && !_ctrlKey ) then {
				["No phone. CTRL+Y to open food menu | ALT+Y to open key menu | CTRL+8 to sync player data", false] spawn doquickmsg;
				_stop = true;
				_handled = true;
				[] spawn life_fnc_AntiSpam2;
			};
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8 && (velocity _veh select 2) < 20) then {
					...
					if(_veh getVariable ["parkingTicket2", false]) then {
						...
						["You have been fined $1500 for incorrect parking!", false] spawn domsg;
					...
					if(_veh getVariable ["parkingTicket", false]) then {
						...
						["You have been fined $500 for incorrect parking!", false] spawn domsg;
