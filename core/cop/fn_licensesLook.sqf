...
if(side _civ != civilian && side _civ != east) exitwith { ["License check failed! Must be Civilian..", false] spawn domsg; };
...
if(side player == west) then {
	...
	["Gathering Information - Please Wait!", false] spawn domsg;
	waituntil {life_gather_info};
};

if(!skip_all) then {
	_message = name _civ;
	_id_name ctrlSetStructuredText parseText format["<img size='0.5' image='\OL_Textures\images\name.paa'/> Name:<t color='#C80000'> %1 </t>",_message];
	...
	_id_faction ctrlSetStructuredText parseText format["<img size='0.5' image='\OL_Textures\images\faction.paa'/> Info:<t color='#C80000'> %1 </t>",_message];

	_message = getPlayerUID _civ;
	_id_uid ctrlSetStructuredText parseText format["<img size='0.5' image='\OL_Textures\images\uid.paa'/> ID#:<t color='#C80000'> %1 </t>",_message];

	_id_city ctrlSetStructuredText parseText format["<img size='0.5' image='\OL_Textures\images\city.paa'/> Residence:<t color='#C80000'> Lakeside </t>",_message];

	_message = count target_houses;
	_id_houses ctrlSetStructuredText parseText format["<img size='0.5' image='\OL_Textures\images\houses.paa'/> House Count:<t color='#C80000'> %1 (Shown on Map)</t>",_message];

	_message = target_licenses;
	_id_licenses ctrlSetStructuredText parseText format["<img size='0.5' image='\OL_Textures\images\license.paa'/> Licenses:<t color='#C80000'> %1 </t>",_message];
	...
	["License check failed!", false] spawn domsg;
};
