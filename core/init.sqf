...
["Waiting for the server to be ready...", false] spawn domsg;

waitUntil{!isNil "life_server_isReady"};
waitUntil{(life_server_isReady OR !isNil "life_server_extDB_notLoaded")};
if (!isNil "life_server_extDB_notLoaded") exitWith { ["The server-side extension extDB was not loaded into the engine, report this to the server admin.", false] spawn domsg };
[] call SOCK_fnc_dataQuery;
waitUntil {life_session_completed};
["Server Ready! Finishing client setup.", false] spawn domsg;
...
fnc_checkPhone = {
	...
	if(count _radios > 0) then {
		if([(call TFAR_fnc_activeSwRadio),"OL_Tablet"] call TFAR_fnc_isSameRadio) then {
			...
			player additem "OL_Tablet";
			player assignitem "OL_Tablet";
		};
	} else {
		player additem "OL_Tablet";
		player assignitem "OL_Tablet";
...
_characterInfo = [] spawn { [player] remoteExec ["LS_fnc_CharacterRequest", call life_fnc_HCC] };
waitUntil {!PLRP_FirstSpawn};
